...
app.get("/api/customers/find", async (req, res) => {
  const { customerName } = req.query;

  try {
    // Find customer by customerName
    const customer = await Bills.findOne({ customerName });
...
app.put("/api/users/make-admin/:id", async (req, res) => {
  try {
    ...
    // Use findOneAndUpdate to update the user's role
    const updatedUser = await Users.findOneAndUpdate(filter, update, { new: true });

    // Log and check if the user was found
    if (!updatedUser) {
